...
def run_script(script_path):
    try:
        ...
        process = subprocess.Popen(["/users/pgrad/singhr6/protocol_work/protocolwork/bin/python3", script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
scripts = ["colour.py", "gps.py", "cpu_temperature.py"]
...
print("It's midnight! Launching scripts...")

# Start each script in a separate thread
for script in scripts:
    thread = threading.Thread(target=run_script, args=("/users/pgrad/singhr6/group24/" + script,))
    thread.start()
